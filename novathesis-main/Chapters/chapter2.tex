...
\section{Descrição Técnica IVML (Ainda não implementei)} % (fold)
\label{sec:esp_ivml}
% Tipos de Diagramas da IVML -> Componentes da Linguagem -> Exemplo de Aplicação -> Limitações da Linguagem

%PARTE ROXA DO DOCUMENTO
A criação da linguagem \gls{IVML} surge para dar suporte à documentação e modelação de \textit{dashboards} analíticos. Atualmente, as ferramentas mais utilizadas para a visualização e implementação de \textit{dashboards} concentram as suas funcionalidades exclusivamente nas funcionalidades a nível visual e de representação, deixando de parte a parte da prototipagem e documentação formas das interações, tooltips e estrutura analítica que esta possui.

%PARTE AZUL DO DOCUMENTO
Neste contexto, a \gls{IVML} apresenta-se como uma proposta 

A \gls{IVML} é composta por dois tipos de diagramas: o \textbf{diagrama de interações}, que representa a estrutura do \textit{dashboard} e as interações entre as suas componentes; e o \textbf{diagrama de tooltips}, que descreve visualmente o comportamente das tooltips presentes em cada \textit{dashboard}. Esta separação entre diferentes diagramas promove a modularidade e evita a sobrecarga visual quando lidamos com \textit{dashboards} de alta complexidade.



% Fazer uma overview (professor referiu que não é suposto ser muito grande, referenciando 3/4 paginas) que inclua
% Qual o problema que a linguagem pretende enfrentar, como é que a linguagem o vai resolver
% O Objetivo da linguagem (parte documental que pretende documentar o layout, documentação das interações e tooltips que não são fácies de ver)
% As principais componentes da linguagem (aqui ajudar a explicação com um exemplo)
% Limitações que a linguagem enfrenta
