...
Os \textit{dashboards} \textbf{analíticos} partilham atributos comuns com os \textit{dashboards} estratégicos e operacionais, uma vez que podem apresentar informação consuante um grande espaço temporal como as \textit{dashboards} estratégicas, e informação detalhada como as \textit{dashboards} operacionais. No entando, o principal propósito deste tipo de \textit{dashboard} é a decoberta de padrões e tendências nos dados, através da constante interação e exploração de informação, consuante diferentes janelas temporais.
...
% Aqui no notebooklm eles falaram que um documento suporta o contar historias na dashboard, o que melhorou bastante a compreensão e reduziu a quantidade de erros tomados
% Posso fazer como o Ferreira fez no capitulo 1.2 Descrição do problema onde falo sobre o documento "10 principios..." e apenas falo sobre as 3 categorias que ele falou e exploro cada uma em detalhe com exemplos e tal

%Fazer alguma referência ao que é dito em cima
%Introduzir os conceitos e recomendações de design de dashboards (aqui secalhar posso citar uma coisa ou outra do novo docjmento que o stor mandou)
%listar as 3 técnicaas que o documento fala
...
\subsection{Ferramentas para Desenho de Dashboards} % (fold)
...
O \textbf{\textit{Google Data Studio}} \cite{googleStudio} é uma ferramenta \textit{cloud-based} que ajuda na criação colaborativa de \textit{dashboards} e relatório interativos. Através da sua integração com serviçoes Google, esta ferramenta torna-se muito utilizada por equipas que procuram rapidez, acessibilidade e colaboração em tempo real \cite{richardson2020magic}.

Por fim, o \textbf{\textit{Qlik Sense}} \cite{qlikSense} é uma plataforma que utiliza um motor associativo para efetuar a exploração dos dados. O seu motor associativo oferece sugestões associadas ao contexto em que os dados são inseridos. Este ferramente oferece uma interface intuitiva, facilitando a descoberta e geração automatíca de \textit{insights} \cite{richardson2020magic}.

\begin{comment}
O \textbf{\textit{Tableau}} \cite{tableau} é também uma das ferramentas mais reconhecidas e utilizadas no mercado, reconhecida pela sua interface intuitiva baseada em \textit{drag-and-drop}. Esta ferramente permite a criação de \textit{dashboards} interativos e personalizáveis, com a capacidade de se conectar a uma ampla variedade de fontes de dados e suporte para análises avançadas \cite{richardson2020magic}.

O \textbf{\textit{Google Data Studio}} \cite{googleStudio} é uma ferramenta \textit{cloud-based} que facilita a criação de \textit{dashboards} e relatórios interativos. A sua escolha é muitas vezes motivada pela sua integração com outras ferramentas Google \cite{richardson2020magic}.
...
%\subsection{Princípios de Usabilidade e Experiência do Utilizador em Interfaces Interativas (Ainda não implementei, secalhar não faz sentido implementar)} % (fold)
%\label{sub:principios_usabilidade}
%ta guardado no consensus o que devo falar aqui neste capitulo
%Neste capítulo irei falar sobre os princípios de usabilidade e experiência do utilizador em interfaces interativas.

%Este subcapítulo permitir-te-á alargar a análise para além de Sridevi, abordando conceitos-chave da usabilidade e da experiência do utilizador (UX) aplicados a interfaces interativas, sem depender ainda do IVML Designer.

%Neste subcapítulo poderás explorar:
%  Heurísticas de Nielsen (como visibilidade do estado do sistema, liberdade de controlo, prevenção de erros, flexibilidade, estética e design minimalista);
%  Design centrado no utilizador (UCD) como filosofia de base;
%  O papel da feedback visual e acessibilidade em interfaces gráficas;
%  A importância da hierarquia visual, affordances, consistência e mapeamento natural.

%Com esta abordagem, solidificas o capítulo com uma base teórica transversal e reconhecida academicamente, que mais tarde poderá ser retomada quando introduzires o IVML Designer no Capítulo 3.
...
\section{Descrição Técnica IVML} % (fold)
\label{sec:esp_ivml}
% Tipos de Diagramas da IVML -> Componentes da Linguagem -> Exemplo de Aplicação -> Limitações da Linguagem
...
% basear me no documento para expandir melhor o que é cada componente
...
%\begin{figure}[htbp]
%  \includegraphics[width=\textwidth]{ivml/diagrama_interacao}
%  \centering
...
%\begin{figure}[htbp]
%  \includegraphics[width=\textwidth]{ivml/diagrama_tooltip}
%  \centering
...
\begin{figure}[htbp]
  ...
  \begin{subfigure}[b]{0.45\textwidth}
    \includegraphics[height=2.5in, width=\textwidth]{ivml/diagrama_interacao}
    \caption{Exemplo de diagrama de interações.}
    \label{fig:interacao}
  \end{subfigure}
  \hfill
  \begin{subfigure}[b]{0.45\textwidth}
    \includegraphics[height=2.5in, width=\textwidth]{ivml/diagrama_tooltip}
    \caption{Exemplo de diagrama de tooltips.}
    \label{fig:tooltip}
  \end{subfigure}
...
\begin{figure}[htbp]
  \includegraphics[width=\textwidth]{ivml/exemplo_real}
  \centering
  \caption{\textit{Dashboard} de exemplo do \textit{Tableau} para o conjunto de dados "\textit{Superstore}" \cite{Ferreira2023IVML}.}
  \label{fig:exemplo}
\end{figure}

% Aqui agora é fazer uma descrição breve sobre as componentes de cada diagrama
